...
import { JhipsterObjectUtils } from './object_utils';

export class JhipsterFormatUtils {
   ...
   */
  public static formatComment(comment) {
    if (!comment) {
      return undefined;
    }
  ...
  private static defaultsForLiquibaseDateFormatting() {
    return {
      date: new Date(),
      increment: 0
    };
  ...
  public static dateFormatForLiquibase(args?) {
    if (args && args.date) {
      // to safely handle the date, we create a copy of the date
      args.date = new Date(JSON.parse(JSON.stringify(args.date)));
    }
    const merged = JhipsterObjectUtils.merge(JhipsterFormatUtils.defaultsForLiquibaseDateFormatting(), args);
